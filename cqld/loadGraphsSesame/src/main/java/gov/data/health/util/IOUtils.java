...
public class IOUtils {
   ...
   * @return The n-tuples concatenated.
   */
  public static Map<String,StringBuilder> sortAndMapNTriples(
		  final String[] values, Map<String,StringBuilder> map) {
	  if ( map==null ) {
		  map = new TreeMap<String, StringBuilder>();
	  }
    mapNTriples(values, map);
    return map;
  }
   ...
   * @return The n-tuples concatenated.
   */
  private static void mapNTriples(final String[] values, final Map<String,StringBuilder> map) {
    for (final String value : values) {
      if (value != null) {
        final int firstWhitespace = value.indexOf(' ');
        final int secondWhitespace = value.indexOf(' ', firstWhitespace + 1);
        ...
        final String key = value.substring(0, firstWhitespace);
        final String object = value.substring(0, lastDot - 1);
        StringBuilder tb = map.get(key);
        if (tb == null) {
          tb = new StringBuilder();
          tb.append(key);
