...
<xsl:variable name="lat-uri"       select="concat($vcard, 'latitude')" />
<xsl:variable name="long-uri"      select="concat($vcard, 'longitude')" />
<xsl:variable name="label-uri"     select="concat($rdfs,  'label')" />
<xsl:variable name="prefLabel-uri" select="concat($vcard, 'address')" />
<xsl:variable name="altLabel-uri"  select="concat($vcard, 'city')" />
<xsl:variable name="name-uri"      select="concat($vcard, 'label')" />
<xsl:variable name="title-uri"     select="concat($vcard, 'zip')" />
...
<xsl:template name="substituteParam">
	...
	<xsl:choose>
		<xsl:when test="$value != '' and 
			((contains($uri, concat('&amp;', $paramNameValue)) and
